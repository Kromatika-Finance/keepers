    ...
    for(let i = 0; i < supportedChains.length; i++){
        ...
        try{
            ...
            if(receipt.upkeepNeeded == true){
                const result = await contract.methods.performUpkeep(receipt.performData);
                console.log(result);
                
                
                const content = '\n\nOrder processed successfully on: ' + supportedChains[i] + ' on ' + getCurrentTime() ;
                fs.appendFile('successful-orders.txt', content, err => {
                    if(err) console.log('ERROR APPENDING FILE');
                    else console.log('file successfully apended;')
                })
            }
        ...
        console.log("Date&Time: " , getCurrentTime())
    }
